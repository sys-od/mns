...
# Author: sys-od
# Source: https://github.com/sys-od/mns

# TODO:
# - AUTO_IPV6=1 => auto choose/add a IPv6 when creating on mns install
# - mns install <prof_name> -c NUMBER | --count=NUMBER ?? => privkey array, print "MN + dupe_offset : privkey[i]"
# - check and test memory reduction .conf parameters
# - check mns list [prof] JSON API bugs on unexpected MN values
# - pid003 support
...
function load_profile() {
	...
	local -A prof=$(get_conf .mns/$1)
	local -A conf=$(get_conf .mns/mns.conf)

	local CMD_ARRAY=(COIN_NAME COIN_DAEMON COIN_CLI COIN_FOLDER COIN_CONFIG)
	for var in "${CMD_ARRAY[@]}"; do
		if [[ ! "${!prof[@]}" =~ "$var" || -z "${prof[$var]}" ]]; then
			echo -e "Seems like you modified something that was supposed to remain unmodified: ${MAGENTA}$var${NC} parameter should exists and have a assigned value in ${GREEN}.mns/$1${NC} file"
	...
	if [[ ! "${!conf[@]}" =~ "$1" || -z "${conf[$1]}" || ! $(is_number "${conf[$1]}") ]]; then
		echo -e "Seems like you modified something that was supposed to remain unmodified: ${MAGENTA}$1${NC} parameter should exists and have a assigned number in ${GREEN}.mns/mns.conf${NC} file"
		echo -e "You can fix it by adding ${MAGENTA}$1=0${NC} to the .mns/mns.conf file (replace the number 0 for the number of nodes installed with mns using the ${BLUE}$1${NC} profile)"
		echo_json "{\"error\":\"mns.conf modified\",\"errcode\":102}"
		exit
	fi
...
function configure_systemd() {
	...
	if [[ $1 && ! "$(ps axo cmd:100 | egrep $service_name)" ]]; then
		echo -e "\n${RED}IMPORTANT!!!${NC} \
			...
			\nThe most common causes of this might be that either you made something to a file that mns modifies or creates, or that you don't have enough free resources (usually memory).
...
function cmd_profadd() {
	...
	[[ ! -d ~/.mns ]] && mkdir ~/.mns
	[[ ! -f ~/.mns/mns.conf ]] && touch ~/.mns/mns.conf
	[[ $(conf_get_value ~/.mns/mns.conf $prof_name) ]] || $(conf_set_value ~/.mns/mns.conf $prof_name 0 1)
	...
	if [[ ${fix_path:${#fix_path}-1:1} != "/" ]]; then
		sed -i "/^COIN_PATH=/s/=.*/=\"${fix_path//"/"/"\/"}\/\"/" ~/.mns/$prof_name
	fi
	if [[ ${fix_folder:${#fix_folder}-1:1} == "/" ]]; then
		fix_folder=${fix_folder::-1}
		sed -i "/^COIN_FOLDER=/s/=.*/=\"${fix_folder//"/"/"\/"}\"/" ~/.mns/$prof_name
	fi

	echo -e "${BLUE}$prof_name${NC} profile successfully added, use ${GREEN}mns install $prof_name${NC} to create a new instance of the masternode"

	local retcode=0
	if [[ -z "${prof[COIN_SERVICE]}" ]]; then
		echo -e "\n${YELLOW}WARNING:${NC} The provided profile doesn't have a ${CYAN}\"COIN_SERVICE\"${NC} parameter, the mns script won't be able to stop the main node on some commands"
		if [[ -t 1 ]]; then
		...
		if [[ -f /etc/systemd/system/${prof[COIN_NAME]}.service ]]; then
			...
			conf_set_value ~/.mns/$prof_name "COIN_SERVICE" "\"${prof[COIN_NAME]}.service\"" 1
			retcode=1
...
function cmd_install() {
	...
	$(conf_set_value .mns/mns.conf $PROFILE_NAME $1 1)
	...
	if [[ $INSTALL_BOOTSTRAP ]]; then
		if [[ ! $(wallet_cmd loaded) || ($COIN_SERVICE && -f /etc/systemd/system/$COIN_SERVICE) ]]; then
			...
			echo -e "${YELLOW}WARNING:${NC} Can't automatically apply the bootstrap, use ${GREEN}mns bootstrap${NC} manually"
			((retcode+=1))
		fi
	fi
	...
	echo -e "===================================================================================================\
			...
			\nRPC port is ${MAGENTA}$NEW_RPC${NC}, this one is used to send commands to the wallet, DON'T put it in 'masternode.conf' (other programs might want to use this port which causes a conflict, but you can change it with ${MAGENTA}mns rpcchange $PROFILE_NAME $1 PORT_NUMBER${NC}).\
			...
			\nNOTE 1: ${GREEN}$COIN_CLI-0${NC} and ${GREEN}$COIN_DAEMON-0${NC} are just a reference to the 'main masternode', not a created one with mns.\
...
function cmd_reinstall() {
	...
	$(conf_set_value .mns/mns.conf $PROFILE_NAME $DUP_COUNT)
}
function cmd_uninstall() {
	...
	if [ $1 == "all" ]; then
		...
		$(conf_set_value .mns/mns.conf $PROFILE_NAME 0 1)
		$(make_chmod_file /usr/bin/$COIN_CLI-all    "#!/bin/bash\nfor (( i=0; i<=0; i++ )) do\n echo -e MN\$i:\n $COIN_CLI-\$i \$@\ndone")
		$(make_chmod_file /usr/bin/$COIN_DAEMON-all "#!/bin/bash\nfor (( i=0; i<=0; i++ )) do\n echo -e MN\$i:\n $COIN_DAEMON-\$i \$@\ndone")
		systemctl daemon-reload
		...
		$(make_chmod_file /usr/bin/$COIN_DAEMON-all "#!/bin/bash\nfor (( i=0; i<=$(($DUP_COUNT-${#del_list[@]})); i++ )) do\n echo -e MN\$i:\n $COIN_DAEMON-\$i \$@\ndone")
		$(conf_set_value .mns/mns.conf $PROFILE_NAME $(($DUP_COUNT-${#del_list[@]})) 1)
...
function cmd_bootstrap() {
	...
	if [[ $1 -eq $2 ]]; then
		...
		if [[ ! $COIN_SERVICE || ! -f /etc/systemd/system/$COIN_SERVICE ]]; then
			...
			[[ $2 -eq 0 ]] && echo -e "Optionally you can put use a dupe as source of the chain files, example: ${YELLOW}mns bootstrap PROFILE 2 1${NC} (copy dupe 1 to dupe 2)."
...
function cmd_ipmod() {
	...
	if [[ ! $netmask ]]; then
		...
		echo -e "${RED}ERROR:${NC} Interface ${GREEN}$4${NC} doesn't exists, use ${YELLOW}mns iplist${NC} to see the existing interfaces"
		echo_json "{\"error\":\"Interface doesn't exists\",\"errcode\":702}"
		return
	elif [[ $(echo "$iface" | wc -l) -gt 1 ]]; then
		echo -e "${RED}ERROR:${NC} There are 2 or more available interfaces, you'll have to specify it as an extra parameter, use ${YELLOW}mns iplist${NC} to see the existing interfaces"
	...
	if [[ ! $ip_res ]]; then
		touch /etc/init.d/mns_ipmanage
		local initd=$(cat /etc/init.d/mns_ipmanage | grep "^ip")
		if [[ $1 == "add" ]]; then
		...
		echo -e "#!/bin/sh -e\
		\n### BEGIN INIT INFO\
		\n# Provides:          mns_ipmanage\
		\n# Required-Start:    \$remote_fs \$syslog\
		\n# Required-Stop:     \$remote_fs \$syslog\
		\n# Default-Start:     5\
		\n# Default-Stop:\
		\n# Short-Description: Start ips at boot time\
		\n# Description:       mns script ip manager to keep ips enabled after reboot\
		\n### END INIT INFO\
		\n\
		\n$initd\
		\n\
		\nexit 0" > /etc/init.d/mns_ipmanage
		chmod +x /etc/init.d/mns_ipmanage
		update-rc.d mns_ipmanage defaults
	...
	if [[ ! $1 ]]; then
		...
		for prof in $profs; do
			...
			if [[ $check_prof ]]; then
				...
				load_profile $prof 1
				local js_dupes=()
				echo -e "${BLUE}$prof${NC}: ${CYAN}$DUP_COUNT${NC} created nodes with mns"
...
function cmd_swapfile() {
	...
	[[ -f /mnt/mns_swapfile ]] && swapoff /mnt/mns_swapfile &> /dev/null

	if [[ $1 -eq 0 ]]; then
		rm -rf /mnt/mns_swapfile
		sed -i "/\/mnt\/mns_swapfile/d" /etc/fstab
		...
		dd if=/dev/zero of=/mnt/mns_swapfile bs=1024 bs=1M count=$1 status=progress
		chmod 600 /mnt/mns_swapfile &> /dev/null
		mkswap /mnt/mns_swapfile &> /dev/null
		swapon /mnt/mns_swapfile &> /dev/null
		/mnt/mns_swapfile swap swap defaults 0 0 &> /dev/null
		[[ ! $(cat /etc/fstab | grep "/mnt/mns_swapfile") ]] && echo "/mnt/mns_swapfile none swap 0 0" >> /etc/fstab
...
function cmd_help() {
	echo -e "Options:\
			\n  - ${YELLOW}mns profadd <prof_file> [prof_name]          ${NC}Adds a profile that will be used to create duplicates of the masternode, it will use the COIN_NAME parameter as name if a prof_name is not provided.\
			\n  - ${YELLOW}mns profdel <prof_name>                      ${NC}Deletes the given profile name, this will uninstall too any dupe that uses this profile.\
			\n  - ${YELLOW}mns install <prof_name> [params...]          ${NC}Install a new dupe based on the parameters of the given profile name.\
			...
			\n  - ${YELLOW}mns reinstall <prof_name> <node> [params...] ${NC}Reinstalls the specified dupe, this is just in case if the dupe is giving problems.\
			\n      ${YELLOW}[params...]${NC} list:\
			\n        ${GREEN}-i ${DARKCYAN}IP${NC},  ${GREEN}--ip=${DARKCYAN}IP${NC}       Use a specific IPv4 or IPv6.\
			\n        ${GREEN}-r ${DARKCYAN}RPC${NC}, ${GREEN}--rpcport=${DARKCYAN}RPC${NC} Use a specific port for RPC commands (must be valid and not in use).\
			\n        ${GREEN}-p ${DARKCYAN}KEY${NC}, ${GREEN}--privkey=${DARKCYAN}KEY${NC} Set a user-defined masternode private key.\
			\n        ${GREEN}-b${NC},     ${GREEN}--bootstrap${NC}   Apply a bootstrap during the reinstallation.\
			\n  - ${YELLOW}mns uninstall <prof_name> <node...|all>      ${NC}Uninstall the specified node/s of the given profile name, you can put ${YELLOW}all${NC} instead of a node number/s to uninstall all the duplicated instances.\
			\n  - ${YELLOW}mns bootstrap <prof_name> <node_1> <node_2>  ${NC}Copies the chain from node_1 to node_2.\
			\n  - ${YELLOW}mns iplist                                   ${NC}Shows all your configurated IPv4 and IPv6.\
			\n  - ${YELLOW}mns ipadd <ip> <netmask> [interface]         ${NC}Allows the system to recognize a new IPv4 or IPv6.\
			\n  - ${YELLOW}mns ipdel <ip> <netmask> [interface]         ${NC}Deletes an already recognized IPv4 or IPv6.\
			\n  - ${YELLOW}mns rpcchange <prof_name> <node> [port]      ${NC}Changes the RPC port used from the given node with the new one (or finds a new one by itself if no port is given).\
			\n  - ${YELLOW}mns systemctlall <prof_name> <command>       ${NC}Applies the systemctl command to all the duplicated instances of the given profile name.\
			\n  - ${YELLOW}mns list [prof_names...] [params...]         ${NC}Shows the amount of duplicated instances of every masternode, if a profile name/s are provided, it lists an extended info of the profile/s instances.\
			\n      ${YELLOW}[params...]${NC} list:\
			...
			\n  - ${YELLOW}mns checkmem                                 ${NC}Shows the memory usage (%) of every group of nodes.\
			\n  - ${YELLOW}mns swapfile <size_in_mbytes>                ${NC}Creates, changes or deletes (if parameter is ${CYAN}0${NC}) a swapfile of the given size in MB to increase the virtual memory.\
			\n  - ${YELLOW}mns update                                   ${NC}Checks the last version of the script and updates it if necessary.\
			...
			\n**NOTE 2**: Check ${CYAN}https://github.com/sys-od/mns/wiki/Commands${NC} for extended info and usage examples of each command.\
			\n**NOTE 3**: Check ${CYAN}https://github.com/sys-od/mns/wiki/FAQs${NC} for technical questions and troubleshooting."
}
function cmd_update() {
	curl -sL https://raw.githubusercontent.com/sys-od/mns/master/mns_install.sh 3>&3 | bash
	exit
}
...
function main() {
	...
	if [[ ! $1 ]]; then
		echo -e "No command inserted, use ${YELLOW}mns help${NC} to see all the available commands"
	...
	case "$1" in
		"profadd")
			exit_no_param "$2" "${YELLOW}mns profadd <prof_file> [prof_name]${NC} requires a profile file and optionally a new profile name as parameters"
			cd $curr_dir
			cmd_profadd $2 $3
			;;
		"profdel")
			exit_no_param "$2" "${YELLOW}mns profadd <prof_name>${NC} requires a profile name as parameter"
			load_profile $2
			cmd_profdel
			;;
		"install")
			exit_no_param "$2" "${YELLOW}mns install <prof_name> [params...]${NC} requires a profile name of an added profile as a parameter"
			load_profile $2 1
			opt_install_params "${@:3}"
			cmd_install $(($DUP_COUNT+1))
			;;
		"reinstall")
			exit_no_param "$3" "${YELLOW}mns reinstall <prof_name> <node> [params...]${NC} requires a profile name and a node number as parameters"
			load_profile $2 1
			instance_valid $3
			...
			;;
		"uninstall")
			exit_no_param "$3" "${YELLOW}mns uninstall <prof_name> <node...|all>${NC} requires a profile name and a node number/s (or all) as parameters"
			load_profile $2
			[[ ! ${@:3} =~ "all" ]] && for arg in ${@:3}; do instance_valid $arg; done
			cmd_uninstall $([[ ${@:3} =~ "all" ]] && echo "all" || echo $(for x in ${@:3}; do echo $(stoi $x); done))
			;;
		"bootstrap")
			exit_no_param "$4" "${YELLOW}mns bootstrap <prof_name> <node_1> <node_2>${NC} requires a profile name and 2 node numbers as parameters"
			load_profile $2 1
			instance_valid $3 1
			instance_valid $4 1
		...
		"iplist")
			...
			;;
		"ipadd")
			exit_no_param "$3" "${YELLOW}mns ipadd <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "add" $2 $3 $4
			;;
		"ipdel")
			exit_no_param "$3" "${YELLOW}mns ipdel <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "del" $2 $3 $4
			;;
		"rpcchange")
			exit_no_param "$3" "${YELLOW}mns rpcchange <prof_name> <node> [port]${NC} requires a profile name, node number and optionally a port number as parameters"
			load_profile $2 1
			instance_valid $3 1
			cmd_rpcchange $(stoi $3) $4
			;;
		"systemctlall")
			exit_no_param "$3" "${YELLOW}mns systemctlall <prof_name> <command>${NC} requires a profile name and a command as parameters"
			load_profile $2
			cmd_systemctlall $3
			;;
		"list")
			...
			;;
		"swapfile")
			exit_no_param "$2" "${YELLOW}mns swapfile <size_in_mbytes>${NC} requires a number as parameter"
			cmd_swapfile $(stoi $2)
			;;
		"checkmem")
			cmd_checkmem
		...
		*)
			...
			echo -e "use ${YELLOW}mns help${NC} to see all the available commands"
